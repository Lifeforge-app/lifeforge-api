...
const schemaFiles = fs.readdirSync("./src", { recursive: true }).filter((file) => typeof file === "string" && file.endsWith("schema.json"))

console.log(`Found ${schemaFiles.length} schema files. Importing...`)

const allSchemas = schemaFiles.map((file) => {
    return JSON.parse(fs.readFileSync
        (`./src/${file}`, { encoding: "utf-8" }))
}).flat()
