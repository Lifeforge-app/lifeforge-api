  ...
  asyncWrapper(async (req, res: Response<BaseResponse<IGuitarTabsEntry>>) => {
    ...
    try {
      ...
      for (let i = 0; i < pics.length; i++) {
        ...
        fs.writeFileSync(`./uploads/${id}/${i}.jpg`, Buffer.from(arrayBuffer));
      }

      const doc = new PDFDocument({ autoFirstPage: false });
      const writeStream = fs.createWriteStream("./uploads/" + id + ".pdf");
        ...
        try {
          ...
          const newEntry = await pb
            ...
            .create<IGuitarTabsEntry>({
              ...
              pdf: new File(
                [fs.readFileSync(`./uploads/${id}.pdf`)],
                `${id}.pdf`
              ),
              type: (() => {
                switch (category) {
              ...
              thumbnail: new File(
                [fs.readFileSync(`./uploads/${id}/0.jpg`)],
                `${id}.jpeg`
              ),
            });
